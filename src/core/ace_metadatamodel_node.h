...
namespace Ace
{
    ...
    * reason this class must exist as a wrapper around the metadata class is because 
    * the model must work with pointers for arrays and objects while the metadata 
    * classes do not use pointers. The node can be any possible metadata class. If it 
    * is an array or object it uses its own custom qt containers that hold pointers 
    * instead of metadata arrays or objects. The nodes are also Qt Objects to allow 
   ...
   private:
      /*!
       * Internal array of node pointers that is used to contain this node's children if 
       * it is an array type. If this node is not an array type this will always be 
       * empty. 
       */
      QList<Node*> _array;
      /*!
       * Internal mapping of node pointers that is used to contain this node's children 
       * if it is an object type. If this node is not an object type this will always be 
       * empty. 
       */
      QMap<QString,Node*> _map;
      /*!
       * Metadata object used to store the metadata type of this node. It is also used to 
       * store the value of basic types that are not arrays or objects. It is NOT used to 
       * store the children of arrays or objects. 
       */
      EMetadata _meta;
   };
}
