...
namespace Ace
{
   ...
   namespace Analytic
   {
       ...
       * node that is not rank 0 and by definition a slave node. The sole function of
       * this class is to process work blocks given to it by the master node and
       * return the corresponding result blocks until a termination code is received.
       * This class also sends a special code to the master node to signal when it is
       * ready to start processing work blocks and if it is serial or accelerated.
       * This special signal to the master node is not sent until it is given the
       * resource type it will use by having the interface mpi start called from its
      ...
      private:
         ...
         int _workSize {0};
         /*!
          * True if this slave node has received the terminate signal from the master
