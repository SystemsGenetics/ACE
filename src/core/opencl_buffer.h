...
namespace OpenCL
{
    ...
    * This class provides methods for mapping the OpenCL buffer memory to a
    * temporary host pointer and can be accessed through other helper methods that
    * provides range checking or through a raw memory pointer. Other methods are
    * provided for getting the size of the buffer and the underlying OpenCL memory
    * ID used by other OpenCL classes.
    *
   ...
   private:
      ...
      cl_mem* _id {nullptr};
      /*!
       * Host memory pointer to this object's mapped OpenCL buffer. If this object is
       * not mapped then this is null.
       */
      T* _data {nullptr};
      /*!
       ...
       * object's buffer. This is used to unmap the buffer if this object is
       * destroyed.
       */
...
namespace OpenCL
{
   ...
   /*!
    * Returns a reference to the element with the given index in this buffer
    * object's mapped data for writing. If this buffer object is not mapped, the
    * mapping is not for writing, or the index is out of range then an exception is
    * thrown.
    *
   ...
   /*!
    * Constructs a new buffer object set to a new OpenCL buffer with the given
    * context and size.
    *
   ...
   /*!
    * Returns a read only reference to the element with the given index in this
    * buffer object's mapped data for reading. If this buffer object is not mapped,
    * the mapping is not for reading, or the index is out of range then an
    * exception is thrown.
    *
    *
   ...
   /*!
    * Returns the size of this object's OpenCL buffer as the number of elements of
    * the template type defined. If this object is null then -1 is returned.
    *
    * @return The number of elements this buffer object contains or -1 if this
    *         object is null.
    */
   ...
   /*!
    * Maps this object's OpenCL buffer data to a host memory pointer for reading
    * that can be accessed through other methods of this class. The command to map
    * memory is sent to the given command queue and is not complete until the
    * returned event is complete. If this object is null or is already mapped then
   ...
   /*!
    * Maps this object's OpenCL buffer data to a host memory pointer for writing
    * that can be accessed through other methods of this class. The command to map
    * memory is sent to the given command queue and is not complete until the
    * returned event is complete. If this object is null or is already mapped then
    * an exception is thrown.
   ...
   /*!
    * Unmaps this object's OpenCL buffer data from host memory. The command to
    * unmap memory is sent to the given command queue and is not complete until the
    * returned event is complete. If the object is null or is not mapped then an
    * exception is thrown.
    *
    * @param queue Pointer to the command queue used to add the OpenCL command for
    ...
    * If this object's buffer is not mapped then an exception is thrown. Care must
    * be taken to only read or write to this pointer depending on what type of
    * mapping it is. Reading from a write mapping or writing to a read mapping is
    * undefined.
    *
   ...
   /*!
    * Maps this object's OpenCL buffer to host memory using the given command queue
    * to add the command and the given mapping to set it for reading or writing. If
    * this object is null or is already mapped then an exception is thrown.
    *
    ...
    * OpenCL resources, and deletes the OpenCL memory ID variable. This does not
    * set any of this object's pointers to null.
    */
   ...
   /*!
    * Sets this object to a null state by changing all its pointers to null. This
    * does not free any allocated memory those pointers may be pointing to.
    */
