...
ELog& ELog::operator<<(float value)
{
   // Append the given value to this log's message byte array and return a reference
   // of this log entry.
   _message.append(QString::number(static_cast<double>(value)).toUtf8());
   return *this;
}
...
/*!
 * Writes the fully constructed message to the logging system of this
 * application. Uses a mutex to safely write to the logging system inside any
 * thread. If logging is not active this will do nothing. If logging is disabled
 * then this reports the misuse of logging and does nothing else.
 */
ELog::~ELog()
{
